...
procedure rather than an estimate: 15 of 21 XACML policies, 41 of 49 Kyverno policies, and
22 of 22 Cedar policies lie inside, with no policy left undetermined, and what places a
policy outside is one construct per ecosystem rather than general expressiveness. We price
what decidability buys: of 38 mutants of our reference policy, 16 are provably equivalent,
so a suite whose adequacy is complete by construction is reported at 57.9\% by a pipeline
that cannot decide equivalence.
...
\begin{enumerate}
  ...
  \item A decision procedure for fragment membership, and its result on three published
    corpora: 15/21 XACML, 41/49 Kyverno, 22/22 Cedar, nothing undetermined
...
reported inside only when every function it names is on an allowlist of predicates whose
partition is fixed by a literal in the policy, outside when it reads data the policy does
not contain, and undetermined otherwise.
...
\midrule
XACML \cite{xacml30} & 21 & 15 & 6 & 0 & 71\% \\
Kyverno \cite{kyverno} & 49 & 41 & 8 & 0 & 84\% \\
Cedar \cite{cedar2024} & 22 & 22 & 0 & 0 & 100\% \\
\bottomrule
\end{tabular}
\caption{Fragment membership. Nothing is undetermined in any ecosystem, so each share is a
verdict on the whole corpus rather than on the part the procedure understood.}
\label{tab:membership}
\end{table}

What places a policy outside is one construct per ecosystem, not general expressiveness. In
XACML all six are an \texttt{AttributeSelector}, which puts an arbitrary XML document in the
subject. In Kyverno it is a \texttt{context} entry that queries the cluster or a registry, a
CEL call that does the same, or \texttt{now()}. Cedar has no such construct: it cannot read
data the request does not carry, which is consistent with its being designed for automated
analysis \cite{cedar2024}.

\paragraph{Limits.} These are the projects' own test corpora, not deployed policy, and
membership is a property of guards --- a policy inside the fragment gets exact adequacy for
its own decision structure, and its language remains undecidable in general
...
directories, not deployed policy, and three ecosystems chosen by us are not a sample of the
field. The Kyverno predictive experiment has nine policies in its blind arm, and its operator
...
and that condition is met by 15 of 21 published XACML policies, 41 of 49 Kyverno policies and
all 22 Cedar policies we examined. Where it holds, the equivalent-mutant problem is a table
