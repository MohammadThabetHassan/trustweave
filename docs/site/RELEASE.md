...
TrustWeave 0.2.0 is prepared as a source release target. It is **not** published, tagged, signed, or released merely because a pull request is green. The currently published package remains 0.1.1 until the repository owner approves each external publication step.
...
| Production | Create the approved annotated `v0.2.0` tag and dispatch the production PyPI workflow | Trusted publishing and tags require owner authorization |
