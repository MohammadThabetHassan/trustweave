...
| --- | ---: | --- |
| Chain parsing, rendering, and traversal state | 14 | Validation-context arguments, repeated mapping checks after successful type proof, and structurally unreachable duplicate-state handling. The observable non-fail-closed approval mutant was removed by a public regression. |
...
| Near-miss classification rejection | 4 | A case-folded direct lookup whose `get_close_matches` fallback recovers the identical entry at similarity 1.0, and match-count arguments where only the first, highest-scoring element is consumed. |
| Policy-weakening classifier | 19 | Strict parsed-policy invariants make the changed mapping defaults, unique-position ordering forms, non-empty-string predicates, and approval-field filters unreachable or observationally identical; exact proofs are in the inventory. |
...
| **Total retained equivalents** | **147** | **All retain individual exact diffs and code-level rationales in the tracked triage inventory.** |
