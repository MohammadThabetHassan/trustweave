...
| Artifact | Unreleased branch emitted version | Historical handling | Public schema |
|---|---|---|---|
...
| Bundle diff | `trustweave.dev/bundle-diff/v1alpha3` | Published `0.2.3` emits `trustweave.dev/bundle-diff/v1alpha2`; `trustweave.dev/bundle-diff/v1alpha1` and `trustweave.dev/bundle-diff/v1alpha2` remain readable by SARIF and risk normalization. | [`bundle-diff-v1alpha3.schema.json`](../schemas/bundle-diff-v1alpha3.schema.json) |
...
> **Release boundary.** The table describes the checked-out, **unreleased** branch. Published `trustweave==0.2.3` emits `trustweave.dev/bundle-diff/v1alpha2`. The first authorized release that emits `v1alpha3` must use a new version consistent with the minor-version policy, with `0.3.0` as the recorded minimum.
...
Historical v1alpha1 bundles retain their original bounded schema and are not relabeled or augmented. On this unreleased branch, `trustweave diff` accepts either supported bundle version and emits `trustweave.dev/bundle-diff/v1alpha3`, recording `base.bundle_schema_version`, `head.bundle_schema_version`, and a normalized policy-only delta. The strictly bounded v1alpha3 policy delta makes security-relevant changes visible even when current declared flow outcomes do not change: fail-closed to fail-open (`TW-DIFF-004`), default to allow (`TW-DIFF-005`), approval-control removal (`TW-DIFF-006`), approval-binding removal (`TW-DIFF-007`), rule-decision weakening (`TW-DIFF-008`), required-control removal (`TW-DIFF-009`), classification-taxonomy change (`TW-DIFF-010`), and structural rule-set or matching-boundary changes (`TW-DIFF-011`). The structural signal covers added or removed rules, matching predicates, and potentially order-sensitive first-match changes; it requires human review but does not prove that every reported change is insecure or that every possible policy weakening is detected. Regenerate a bundle with the current CLI to migrate it; do not edit its version string in place.
