...
Historical v1alpha1 bundles retain their original bounded schema and are not relabeled or augmented. On this unreleased branch, `trustweave diff` accepts either supported bundle version and emits `trustweave.dev/bundle-diff/v1alpha3`, recording `base.bundle_schema_version`, `head.bundle_schema_version`, and a normalized policy-only delta. The strictly bounded v1alpha3 policy delta makes security-relevant changes visible even when current declared flow outcomes do not change: fail-closed to fail-open (`TW-DIFF-004`), default to allow (`TW-DIFF-005`), approval-control removal (`TW-DIFF-006`), approval-binding removal (`TW-DIFF-007`), rule-decision weakening (`TW-DIFF-008`), required-control removal (`TW-DIFF-009`), and classification-taxonomy change (`TW-DIFF-010`). Regenerate a bundle with the current CLI to migrate it; do not edit its version string in place.
