...
| High-risk scope score | 9,692 killed / 10,324 generated (93.88% killed); gated scope 7722 of 7885 (97.93% killed) |
...
Every survivor from this run has an individual source-level diff and an explicit classification in [`mutation-survivor-triage-v1.json`](mutation-survivor-triage-v1.json). The regenerated inventory records **163 classified survivors**, **0 untriaged survivors**, **160 equivalent mutations**, **3 defensive mutations**, and **0 mutations marked `needs_regression`**. The 10 `policy_weakening.py` survivors are limited to unreachable strict-parser defaults, unique-position ordering forms, and normalization predicates; the inventory preserves their exact diffs and code-level proofs. Each equivalent record preserves a code-level proof that the changed expression is semantically redundant or unreachable under the strict local contracts.
