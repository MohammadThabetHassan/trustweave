...
| Mutation testing | The recorded Linux run killed **9,692 of 10,324** mutants (**93.88%**) across seventeen mutated modules (the sixteen gated high-risk modules plus the ratcheted discovery analyser `code_analysis.py`); the sixteen-module gated scope, which excludes the ratcheted analyser, killed 7,722 of 7,885 (97.93%). | It is a measured high-risk scope, not a package-wide claim and not proof that TrustWeave is secure. |
