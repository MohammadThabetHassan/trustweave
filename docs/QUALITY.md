...
| High-risk mutation analysis | `mutmut run` then `mutmut results` | A Linux-only diagnostic of the configured seventeen-module (sixteen gated, one ratcheted) evidence, configuration, policy, risk, schema, SARIF, CI-coordination, and source-discovery scope; see [the mutation-testing record](MUTATION_TESTING.md). |
...
Maintainers must complete the local checks before pushing a pull-request head and monitor hosted results on that exact pushed SHA. Merge, tagging, signing, TestPyPI/PyPI publication, and GitHub Release creation remain owner-controlled actions and are outside ordinary quality verification. [Maintainer Handoff](archive/MAINTAINER_HANDOFF.md) defines the exact-SHA review record, owner-controlled setting checks, failure response, and recurring assurance-review procedure.
