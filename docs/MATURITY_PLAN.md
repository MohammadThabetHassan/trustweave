...
| Distribution | `0.2.2` is published on PyPI, validated through clean TestPyPI and PyPI installations, and recorded as GitHub Release `v0.2.2` from annotated tag `3b0817e732627a62a18be82e854a58fa085f0922`. Unreleased `0.2.3` candidate workflows build/inspect both distributions in clean temporary environments and request PyPI project attestations. Annotated `v0.2.0` remains immutable unpublished audit evidence at `7232fe3a23d92f50a693903c0a6b7cb92d0a1426`; it was never published and has no GitHub Release. | Publication, tagging, signing, release creation, and observed package-provenance verification remain owner-controlled. |
...
| Resource and provenance controls | Local file/structure/chain/SARIF bounds fail closed; clean wheel/source-distribution checks and TestPyPI-first attestation-generation workflow controls are verified. | Configured generation is not authenticated package provenance until an exact published file passes expected-repository verification. |
...
| Authenticated provenance | Publish an owner-authorized TestPyPI candidate with configured PyPI project attestations, then record exact-file `pypi-attestations` expected-repository verification before production promotion. | “Signed,” “verified,” or “tamper-proof” labels based on workflow configuration or local hashes alone. |
