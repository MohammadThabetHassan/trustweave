...
This inventory records the assurance baseline after the published 0.2.3 enhancement cycle. It distinguishes what TrustWeave demonstrates in the source tree and in the exact observed 0.2.3 package-release record from controls that still require an owner and an external platform, and capabilities the project deliberately excludes.

The inventory is an engineering record, not an external-adoption claim or a general certification. `0.2.3` is the current public package release; its exact verified package evidence is recorded in [Release Evidence 0.2.3](RELEASE_EVIDENCE_0.2.3.md).
...
| Release controls | SHA-pinned workflows, separate build/publish jobs, OIDC trusted publishing, TestPyPI rehearsal, clean installation, annotated tags, GitHub releases, and exact-file provenance verification for `0.2.3`. | The observed provenance claim is limited to the exact `0.2.3` TestPyPI and PyPI wheels. |

## Implemented 0.2.3 assurance work

| Former gap | Implemented control | Present evidence and remaining limit |
| --- | --- | --- |
| Compatibility claims were distributed across prose and tests | Versioned compatibility source, support/deprecation policy, and exact contract validation. | A local validator proves agreement with published package metadata, CI matrix, CLI surface, schemas, and retained reader fixtures. `0.2.3` is the current public release record. |
