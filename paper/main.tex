...
The policy names no classification, source, tool, purpose or capability, so five of the
seven components of Definition~\ref{def:refining}'s subject contribute a single class each,
and Theorem~\ref{thm:quotient}'s bound is $|T| \cdot |A| \cdot 1 \cdot 1 \cdot 1 \cdot 1
\cdot 1 = 12$. All twelve are achieved, and Table~\ref{tab:example} is the whole of
$\sem{P}$ --- a total function on a 12-element quotient of an infinite subject space.
