...
| Result | 6,566 generated mutants; 6,444 killed; 122 survived; 0 without a selected test; 0 timed out; 0 suspicious. |
| High-risk scope score | 98.14% killed (`6,444 / 6,566`) |
...
Every survivor from this run has an individual source-level diff and an explicit classification in [`mutation-survivor-triage-v1.json`](mutation-survivor-triage-v1.json). The regenerated inventory records **122 classified survivors**, **0 untriaged survivors**, **122 equivalent mutations**, **0 defensive mutations**, and **0 mutations marked `needs_regression`**. The 15 `policy_weakening.py` survivors are limited to unreachable dictionary-default and strict-normalization predicate variants; the inventory preserves their exact diffs and code-level proofs. Each equivalent record preserves a code-level proof that the changed expression is semantically redundant or unreachable under the strict local contracts.
