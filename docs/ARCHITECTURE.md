...
| `risk.py` | Normalizes supplied local review findings with `trustweave/fingerprint/v4`, emits v1alpha2 lifecycle and mismatch states, and applies expiry-enforced decisions. | Does not remediate findings, contact ticketing systems, authenticate an approver, or inspect runtime behavior. |
| `policy_review.py` | Reviews ordered-rule shadowing, single or collective, and decisions that require human scrutiny; says when a cover search was declined (`TW-POL-010`). | Does not decide authorization or run a policy in a deployed runtime. |
